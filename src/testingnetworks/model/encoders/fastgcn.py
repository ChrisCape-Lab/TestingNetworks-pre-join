...
from src.testingnetworks.utils import sparse_mx_to_torch_sparse_tensor


class FastGCN(Encoder):

    INPUTS = [DATA.ADJACENCY_MATRIX, DATA.NODE_FEATURES]

    def __init__(self, layers_dims, act=torch.nn.ReLU(), bias=False, device='cpu'):
        super(FastGCN, self).__init__(input_list=FastGCN.INPUTS)
    ...
    def forward(self, sample: dict) -> torch.Tensor:
        ...
        layer_sizes = [int(num_nodes/2) for _ in range(0, len(self.layers))]
        sampler = FastGCNSampler(node_embs, adj_matrix, input_dim=node_embs.shape[1], layer_sizes=layer_sizes)
        node_embs, adj_list, _ = sampler.sampling([i for i in range(0, num_nodes)])

        for i, layer in enumerate(self.layers):
            node_embs = layer.forward(adj_list[i], node_embs)
...
class Sampler:
    def __init__(self, features, adj, **kwargs):
        ...
        self.adj = sp.coo_matrix(adj.to_dense().detach().cpu().numpy()).tocsr()
...
class FastGCNSampler(Sampler):
    ...
    def sampling(self, v):
        ...
        for layer_index in range(self.num_layers-1, -1, -1):
            cur_sampled, cur_support = self._one_layer_sampling(cur_out_nodes, self.layer_sizes[layer_index])
...
class ASGCNSampler(Sampler, torch.nn.Module):
    ...
    def sampling(self, v):
        ...
        for i in range(self.num_layers-1, -1, -1):
            cur_u_sampled, cur_support, cur_var_need = self._one_layer_sampling(cur_out_nodes, output_size=self.layer_sizes[i])
    ...
    def _calc_variance(self, var_need):
        # NOTE: it's useless in this implementation for the three datasets_output
